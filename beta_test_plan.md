...
| F7             | Addon Dev     | Load addon           | Addon is found and loaded (if enabled in project config) automatically via a json manifest, loading a shared library into the project if included                                               |
...
| F7             | Addons can be loaded into the engine with visible logs                              | Addons can be loaded into the engine and their assets be accessed<br>Shared library loads successfully<br>Logs visible with level: INFO, DEBUG, WARN or ERROR  |            |
