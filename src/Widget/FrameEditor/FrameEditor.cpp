...
void FrameEditor::run()
{
    ...
    drawList->AddRectFilled(canvasP0, ImVec2(canvasP0.x + canvasSize.x, canvasP0.y + canvasSize.y), IM_COL32(255, 255, 255, 255)); // White background
    ...
    if (ImGui::IsMouseDown(0)) { // Left mouse button
        ...
        if (mousePos.x > canvasP0.x && mousePos.x < (canvasP0.x + canvasSize.x) &&
        mousePos.y > canvasP0.y && mousePos.y < (canvasP0.y + canvasSize.y)) {
    ...
    for (auto &toLineUp : _drawnPoints) {
        for (size_t i = 1; i < toLineUp.second.size(); i++) {
            ...
            drawList->AddLine(ImVec2(toDraw[i-1].x, toDraw[i-1].y), ImVec2(toDraw[i].x, toDraw[i].y), 
                        IM_COL32((col >> 24) & (0b11111111), (col >> 16) & (0b11111111), (col >> 8) & (0b11111111), col & 0b11111111), thick);
        }
    }
